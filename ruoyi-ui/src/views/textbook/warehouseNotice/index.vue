  ...
  <div class="app-container">
    <el-row :gutter="20" class="mb20">
      ...
      <el-col :span="6">
        <div class="stat-card stat-card-success">
          ...
          <div class="stat-info">
            <div class="stat-num">{{ stats.inbound }}</div>
            <div class="stat-label">进书通知</div>
          </div>
        </div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card stat-card-danger">
          ...
          <div class="stat-info">
            <div class="stat-num">{{ stats.shortage }}</div>
            <div class="stat-label">缺书通知</div>
          </div>
        </div>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading" :data="noticeList" border stripe @row-click="handleRowClick" highlight-current-row style="width:100%">
      ...
      <el-table-column label="类型" align="center" width="90">
        <template slot-scope="scope">
          <el-tag size="mini" :type="bizTypeTag(scope.row.bizType)">{{ bizTypeLabel(scope.row.bizType) }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog title="通知详情" :visible.sync="open" width="550px" append-to-body>
      <div v-if="currentNotice.noticeId">
        ...
        <p style="color: #909399; margin: 0 0 15px 0;">
          <el-tag size="mini" :type="bizTypeTag(currentNotice.bizType)">{{ bizTypeLabel(currentNotice.bizType) }}</el-tag>
...
export default {
  ...
  methods: {
    ...
    bizTypeTag(type) {
      const m = { '1': '', '2': 'warning', '3': 'success', '4': 'danger', '5': 'info', '6': 'danger', '7': 'success', '8': '', '9': 'warning' }
      return m[type] || ''
    },
    bizTypeLabel(type) {
      const m = { '1': '领书单', '2': '采购单', '3': '入库单', '4': '缺书登记', '5': '供应商通知', '6': '库存预警', '7': '班级领书出库', '8': '领书通知发布', '9': '供应商发货' }
      return m[type] || type
    }
  }
}
