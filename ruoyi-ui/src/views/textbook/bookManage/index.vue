<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="bookList" @selection-change="handleSelectionChange" border stripe :row-class-name="tableRowClassName">
      ...
      <el-table-column label="定价" align="center" prop="price" width="70">
        <template slot-scope="scope">
          <span v-if="scope.row.price">¥{{ scope.row.price }}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="适用课程" align="center" prop="courseName" width="110" show-overflow-tooltip />
      <el-table-column label="教材类型" align="center" prop="textbookType" width="90">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.textbook_type" :value="scope.row.textbookType" />
        </template>
      </el-table-column>
      <el-table-column label="信息状态" align="center" prop="infoStatus" width="80">
        <template slot-scope="scope">
    ...
    <el-dialog title="教材信息导入" :visible.sync="importOpen" width="800px" append-to-body :close-on-click-modal="false">
      ...
      <div v-if="importStep === 1 && !previewData">
        <el-upload ref="importUploadRef"
          ...
          <div slot="tip" class="el-upload__tip">仅支持 .xlsx 格式，文件大小不超过 10MB</div>
        </el-upload>
