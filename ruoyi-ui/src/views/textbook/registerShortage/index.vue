<template>
  <div class="app-container">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="出版社" prop="publisher">
                  <el-input v-model="quickAddForm.publisher" placeholder="请输入出版社" />
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
          <el-alert type="info" :closable="false" style="margin-top: 10px;">
            <template slot="default">快速新增的教材基本信息将被录入系统，后续可由库管员补充完善其他字段。</template>
          </el-alert>
    ...
    <el-dialog title="缺书详情" :visible.sync="viewOpen" width="550px" append-to-body>
      <div v-if="viewData.lackId">
        <h3 style="margin: 0 0 10px 0;">{{ viewData.bookName }}</h3>
        <p style="color: #909399; margin: 0 0 15px 0;">ISBN: {{ viewData.isbn }}</p>
        <el-descriptions :column="1" border size="small">
          <el-descriptions-item label="缺书数量">{{ viewData.lackNum }} 本</el-descriptions-item>
          <el-descriptions-item label="紧急程度">
            <el-tag :type="getUrgencyType(viewData.urgency)" size="mini">{{ getUrgencyLabel(viewData.urgency) }}</el-tag>
...
<script>
import { getShortageList, addShortage, cancelShortage } from "@/api/textbook/shortage";
  ...
  methods: {
    ...
    searchBook(query) {
      ...
      if (query) {
        ...
        }).finally(() => { this.bookSearching = false })
      } else {
        this.bookOptions = []
      }
    },
      ...
      this.$refs["quickAddForm"].validate(valid => {
        if (valid) {
          ...
          quickAddBook(this.quickAddForm).then(res => {
            ...
            this.form.bookId = newBook.bookId
    ...
    handleCancel(row) {
      ...
      this.$modal.confirm('是否确认取消该缺书申请?').then(() => {
    ...
    getUrgencyLabel(level) {
      return level === '2' ? '特急' : level === '1' ? '紧急' : '普通'
    },
