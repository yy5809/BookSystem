<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="教材名称" prop="bookName"><el-input v-model="queryParams.bookName" placeholder="请输入教材名�? clearable @keyup.enter.native="handleQuery" /></el-form-item>
      <el-form-item label="紧急程�? prop="urgency">
        <el-select v-model="queryParams.urgency" placeholder="请选择" clearable><el-option label="普�? value="0" /><el-option label="紧�? value="1" /><el-option label="特�? value="2" /></el-select>
      </el-form-item>
      <el-form-item label="状�? prop="handleStatus">
        <el-select v-model="queryParams.handleStatus" placeholder="请选择" clearable><el-option label="未处�? value="0" /><el-option label="已纳入采�? value="1" /><el-option label="已到�? value="2" /><el-option label="已完�? value="3" /></el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="shortageList" border stripe>
      ...
      <el-table-column label="紧急程�? align="center" prop="urgency" width="100"><template slot-scope="scope"><el-tag :type="getUrgencyType(scope.row.urgency)" size="mini">{{ getUrgencyLabel(scope.row.urgency) }}</el-tag></template></el-table-column>
      <el-table-column label="状�? align="center" prop="handleStatus" width="100"><template slot-scope="scope"><el-tag :type="getStatusType(scope.row.handleStatus)" size="mini">{{ getStatusLabel(scope.row.handleStatus) }}</el-tag></template></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="650px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="选择教材" prop="bookId">
          <el-select v-model="form.bookId" filterable remote reserve-keyword :remote-method="searchBook" :loading="bookSearching" placeholder="输入ISBN或书名搜�? style="width: 100%" @change="handleBookSelect">
        ...
        <el-card v-if="showQuickAdd" shadow="hover" style="margin-bottom: 15px;">
          ...
          <el-form ref="quickAddForm" :model="quickAddForm" :rules="quickAddRules" label-width="80px" size="small">
            <el-row :gutter="20"><el-col :span="12"><el-form-item label="ISBN" prop="isbn"><el-input v-model="quickAddForm.isbn" placeholder="10位或13位数�? /></el-form-item></el-col><el-col :span="12"><el-form-item label="书名" prop="bookName"><el-input v-model="quickAddForm.bookName" placeholder="请输入书�? /></el-form-item></el-col></el-row>
            <el-row :gutter="20"><el-col :span="12"><el-form-item label="作�? prop="author"><el-input v-model="quickAddForm.author" placeholder="请输入作�? /></el-form-item></el-col><el-col :span="12"><el-form-item label="出版�? prop="publisher"><el-input v-model="quickAddForm.publisher" placeholder="请输入出版社" /></el-form-item></el-col></el-row>
          </el-form>
          ...
          <div style="text-align: right; margin-top: 10px;"><el-button size="small" @click="showQuickAdd = false">取消新增</el-button><el-button type="primary" size="small" @click="handleQuickAdd" :loading="quickAddLoading">确认新增并继�?/el-button></div>
        </el-card>
        ...
        <el-form-item label="紧急程�? prop="urgency"><el-radio-group v-model="form.urgency"><el-radio label="0">普�?/el-radio><el-radio label="1">紧�?/el-radio><el-radio label="2">特�?/el-radio></el-radio-group></el-form-item>
      ...
      <div slot="footer" class="dialog-footer"><el-button type="primary" @click="submitForm">�?�?/el-button><el-button @click="cancel">�?�?/el-button></div>
    </el-dialog>
    ...
    <el-dialog title="缺书详情" :visible.sync="viewOpen" width="600px" append-to-body>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="紧急程�?><el-tag :type="getUrgencyType(viewData.urgency)">{{ getUrgencyLabel(viewData.urgency) }}</el-tag></el-descriptions-item>
        <el-descriptions-item label="状�?><el-tag :type="getStatusType(viewData.handleStatus)">{{ getStatusLabel(viewData.handleStatus) }}</el-tag></el-descriptions-item>
      ...
      <div slot="footer" class="dialog-footer"><el-button @click="viewOpen = false">�?�?/el-button></div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      loading: true, quickAddLoading: false, bookSearching: false,
      ...
      quickAddRules: {
        isbn: [{ required: true, message: '请输入ISBN', trigger: 'blur' }, { pattern: /^(\d{10}|\d{13})$/, message: 'ISBN格式不正确（10�?3位数字）', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleQuickAdd() {
      this.$refs["quickAddForm"].validate(valid => {
        if (valid) { this.quickAddLoading = true; quickAddBook(this.quickAddForm).then(res => {
          this.$modal.msgSuccess("教材快速新增成�?); this.showQuickAdd = false;
    ...
    submitForm() {
      this.$refs["form"].validate(valid => { if (valid) {
        addShortage(this.form).then(response => { this.$modal.msgSuccess("登记成功，系统已通知库管员处理"); this.open = false; this.getList(); });
      } });
    },
    ...
    handleCancel(row) {
      ...
      this.$modal.confirm('是否确认取消该缺书申�?').then(() => { return cancelShortage(shortageId); }).then(() => { this.getList(); this.$modal.msgSuccess("取消成功"); }).catch(() => { this.$modal.msgError("取消失败，请稍后重试"); });
    },
    ...
    getStatusLabel(status) { return status === '3' ? '已完�? : status === '2' ? '已到�? : status === '1' ? '已纳入采�? : status === '4' ? '已取�? : '未处�?; },
    isPending(row) {
      if (!row) return false
